...
| --- | --- |
| **Frontend** | • [Vue 3](https://vuejs.org/?utm_source=gemini) (Composition API, `<script setup>`)<br>• [TypeScript]<br>• [Tailwind CSS](https://tailwindcss.com/?utm_source=gemini) (Docker Desktop authentic dark styling)<br>• [Lucide Icons] (`lucide-vue-next`)<br>• [Vite] |
| **Backend Runtime Engine** | • [Golang](https://go.dev/?utm_source=gemini) (`backend/main.go` & `backend/runtime/*`)<br>• RESTful API with automated cgroup metrics, simulated process trees, and CLI engine parser.<br>• Proxying configured seamlessly through Vite (`/api` → `http://127.0.0.1:9090`). |
