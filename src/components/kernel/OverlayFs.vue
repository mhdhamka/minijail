...
const activeViewTab = ref<'stack' | 'metrics' | 'audit'>('stack');
const terminalLogs = ref<string[]>([
  '[overlayfs] superblock registered successfully for mount /merged',
  '[vfs] lowerdir layers mapped in read-only mode',
  '[cow] initialized upperdir diff structure and workdir'
]);
...
function pushLog(msg: string) {
  terminalLogs.value.unshift(`[${new Date().toLocaleTimeString()}] ${msg}`);
  if (terminalLogs.value.length > 8) terminalLogs.value.pop();
}
...
function handleModifyFile(path: string) {
  ...
  if (existing) {
    ...
    existing.content = `// Updated via CoW at ${new Date().toLocaleTimeString()}\n` + (existing.content || '');
    selectedFile.value = existing;
    pushLog(`CoW triggered: Copied ${path} from lowerdir into upperdir diff.`);
  } else {
    ...
    pushLog(`CoW mutation: Instantiated upperdir copy for ${path}`);
  }
}
...
function handleDeleteFile(path: string) {
  ...
  pushLog(`Whiteout created: .wh.${path.split('/').pop()} masks lower layer file.`);
}
...
function handleAddFile() {
  ...
  if (exists) {
    ...
    pushLog(`Updated file in container upperdir: ${newFilePath.value}`);
    return;
  }
  ...
  pushLog(`Created new file directly in container upperdir: ${newFilePath.value}`);
}
...
function resetPlayground() {
  ...
  pushLog('Reset OverlayFS playground environment to default state.');
}
</script>
...
<template>
  <div class="space-y-6 font-sans select-none">
    ...
    <!-- Header with Modernized UI Badge -->
        ...
        <div class="flex items-center gap-2.5">
          ...
          <div>
            <h2 class="text-base font-bold text-white font-sans flex items-center gap-2">
              <span>OverlayFS Layer Stacker & Copy-on-Write Playground</span>
              <span class="text-[10px] px-2 py-0.5 rounded font-mono bg-amber-500/10 text-amber-400 border border-amber-500/30">
                VFS Union Active
              </span>
            </h2>
            <p class="text-xs text-slate-400">
              Simulate zero-duplication disk layering, real-time Copy-on-Write (CoW) hooks, and whiteout device masking.
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="bg-[#141A22] border border-[#21262d] rounded-2xl p-4 shadow-lg space-y-3">
        ...
        <input 
          ...
          min="1" 
          max="30" 
          class="w-full accent-amber-400 cursor-pointer h-1.5 bg-[#1F2937] rounded-lg appearance-none"
        />
        <div class="text-[11px] text-slate-400 font-sans">
          All {{ containerCount }} containers share a single read-only OCI base layer.
        </div>
      </div>

      <div class="bg-[#141A22] border border-[#21262d] rounded-2xl p-4 shadow-lg space-y-1">
        ...
        <div class="text-[11px] text-slate-400 font-sans mt-2">
      ...
      <div class="bg-[#141A22] border border-[#21262d] rounded-2xl p-4 shadow-lg space-y-1">
        ...
        <div class="text-2xl font-bold font-mono text-emerald-400 mt-1 flex items-center gap-2">
          <span>{{ overlayFsDiskUsageMb }} MB</span>
          <span class="text-[10px] px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-300 font-mono border border-emerald-500/30">
    ...
    <!-- MAIN WORKBENCH: STACK VIEWS & INSPECTOR -->
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
      
      <!-- LEFT 7 COLS: LAYER STACK (MERGED -> UPPER -> LOWER) -->
      <div class="lg:col-span-7 space-y-4">
        
        <!-- 1. MERGED VIEW -->
        <div class="bg-[#141A22] border border-[#21262d] rounded-2xl p-4 shadow-xl">
          <div class="flex items-center justify-between mb-3 border-b border-[#21262d] pb-2.5">
            <div class="flex items-center gap-2">
              <Eye class="w-4 h-4 text-emerald-400" />
              <h3 class="text-sm font-bold text-white font-sans">1. Merged View (`/merged`)</h3>
            </div>
            <span class="text-[10px] font-mono px-2.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/30">
              Container RootFS
            </span>
          </div>

          <p class="text-xs text-slate-400 mb-3">
            The container process operates exclusively within this unified mount point. Upper layer files shadow lower files transparently.
          </p>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono">
            <div 
              ...
              :class="selectedFile?.path === file.path 
                ...
                : 'bg-[#0D1117] border-[#21262d] text-slate-300 hover:border-slate-500'"
            >
              ...
              <span 
                class="text-[10px] px-2 py-0.5 rounded uppercase font-bold shrink-0 ml-1"
        ...
        <!-- 2. UPPERDIR LAYER -->
        <div class="bg-[#181510] border border-amber-500/30 rounded-2xl p-4 shadow-xl">
          <div class="flex items-center justify-between mb-3 border-b border-amber-500/20 pb-2.5">
            <div class="flex items-center gap-2">
            ...
            <span class="text-[10px] font-mono px-2.5 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/40">
              Ephemeral Container Diff
            </span>
          </div>

          <p class="text-xs text-amber-200/80 mb-3 font-sans">
            Holds runtime modifications. Deleting lower files creates a special Linux whiteout character device (<code class="text-white">.wh.&lt;name&gt;</code>) here.
          </p>
          ...
          <div class="space-y-2 text-xs font-mono">
            <div 
              ...
              :class="selectedFile?.path === file.path ? 'ring-2 ring-amber-400 bg-amber-500/15' : 'bg-[#0D1117] border-[#2A2215]'"
            >
              <div class="flex items-center gap-2.5 truncate">
                <span 
              ...
              <div class="flex items-center gap-2 shrink-0">
                <span v-if="file.isWhiteout" class="text-[10px] text-rose-400 font-bold bg-rose-950/80 px-2 py-0.5 rounded border border-rose-800">
                  WHITEOUT (0:0)
                </span>
        ...
        <!-- 3. LOWERDIR LAYER -->
        <div class="bg-[#111722] border border-cyan-800/50 rounded-2xl p-4 shadow-xl">
          <div class="flex items-center justify-between mb-3 border-b border-cyan-800/30 pb-2.5">
            <div class="flex items-center gap-2">
            ...
            <span class="text-[10px] font-mono px-2.5 py-0.5 rounded bg-cyan-950/80 text-cyan-400 border border-cyan-800/60">
              Immutable Base Image
            </span>
          </div>

          <p class="text-xs text-slate-400 mb-3 font-sans">
            Base layers are shared across all containers. Writing to any file here automatically triggers kernel Copy-on-Write (CoW).
          </p>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono">
            <div 
              ...
              :key="file.path"
              class="p-2.5 rounded-xl bg-[#0A0D12] border border-[#1E2633] flex items-center justify-between"
            >
              <span class="text-slate-300 truncate">{{ file.path }}</span>
              <div class="flex items-center gap-1.5 shrink-0">
                <button 
                  @click="handleModifyFile(file.path)"
                  class="px-2 py-1 rounded-lg text-[10px] bg-amber-950/80 hover:bg-amber-900 text-amber-300 border border-amber-800/60 cursor-pointer font-bold"
                ...
                <button 
                  ...
                  class="px-2 py-1 rounded-lg text-[10px] bg-rose-950/80 hover:bg-rose-900 text-rose-300 border border-rose-800/60 cursor-pointer font-bold"
      ...
      <!-- RIGHT 5 COLS: FILE INSPECTOR, WRITER & KERNEL LOGS -->
      <div class="lg:col-span-5 space-y-4">
        
        <!-- File Inspector Card -->
        <div class="bg-[#141A22] border border-[#21262d] rounded-2xl p-4 shadow-xl flex flex-col justify-between">
          <div>
            <div class="flex items-center justify-between border-b border-[#21262d] pb-2.5 mb-3">
              <div class="flex items-center gap-2">
                <FileCode class="w-4 h-4 text-cyan-400" />
                <h3 class="text-sm font-bold text-white font-sans">File Layer Inspector</h3>
              </div>
              <span v-if="selectedFile" class="text-[10px] font-mono text-slate-400 px-2 py-0.5 rounded bg-[#0D1117] border border-[#21262d]">
                {{ selectedFile.size }}
              </span>
            </div>

            <div v-if="selectedFile" class="space-y-3 font-mono text-xs">
              <div class="p-3 rounded-xl bg-[#0D1117] border border-[#21262d]">
                <div class="text-[10px] text-slate-500 uppercase font-bold">Target Path</div>
                <div class="text-white font-bold mt-0.5 truncate">{{ selectedFile.path }}</div>
              </div>

              <div class="p-3 rounded-xl border" :class="selectedFile.action === 'deleted' ? 'bg-rose-950/30 border-rose-800 text-rose-300' : selectedFile.action === 'modified' || selectedFile.action === 'added' ? 'bg-amber-950/30 border-amber-800 text-amber-300' : 'bg-cyan-950/30 border-cyan-800 text-cyan-300'">
                ...
                <div class="text-[11px] mt-1.5 font-sans text-slate-300 leading-relaxed">
                  <span v-if="selectedFile.action === 'deleted'">
                    The file was deleted in the container. Linux created a character device (0:0) with name <code>.wh.{{ selectedFile.path.split('/').pop() }}</code> to mask the lower layer.
                  </span>
                  <span v-else-if="selectedFile.action === 'modified'">
                    The base image remains untouched! The kernel duplicated this file into <code>upperdir</code> before applying changes.
                  </span>
                  <span v-else-if="selectedFile.action === 'added'">
                    This file exists exclusively inside the running container's writable diff layer.
                  </span>
                  <span v-else>
                    Shared directly from the base image layer. Reads bypass container disk entirely!
                  </span>
                </div>
              </div>

              <!-- Content Preview Box -->
              <div class="p-3 rounded-xl bg-[#0A0D12] border border-[#1E2633] text-[11px] font-mono text-slate-300 max-h-44 overflow-y-auto shadow-inner">
                <pre class="whitespace-pre-wrap">{{ selectedFile.content || '// Binary or standard system file content' }}</pre>
              </div>
            </div>

            <div v-else class="text-xs text-slate-500 text-center py-10 font-mono">
              Click any file in the stack to inspect its OverlayFS resolution.
            </div>
          </div>
        </div>

        <!-- Write to Container Upperdir Form -->
        <div class="bg-[#141A22] border border-[#21262d] rounded-2xl p-4 shadow-xl space-y-3">
          <div class="flex items-center gap-2 border-b border-[#21262d] pb-2.5">
          ...
          <div class="space-y-2.5 text-xs font-mono">
            <div>
              <label class="text-[10px] text-slate-400 uppercase font-bold">File Path:</label>
              <input 
                type="text" 
                v-model="newFilePath" 
                placeholder="/app/config.json"
                class="w-full mt-1 bg-[#0D1117] border border-[#2B3545] rounded-xl px-3 py-2 text-white focus:outline-none focus:border-blue-500"
              />
            </div>

            <div>
              <label class="text-[10px] text-slate-400 uppercase font-bold">File Content:</label>
              <textarea 
                v-model="newFileContent" 
                rows="3"
                class="w-full mt-1 bg-[#0D1117] border border-[#2B3545] rounded-xl p-2.5 text-white font-mono text-xs focus:outline-none focus:border-blue-500"
              ></textarea>
            </div>

            <button 
              @click="handleAddFile"
              class="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-md"
            >
        ...
        <div class="bg-[#141A22] border border-[#21262d] rounded-2xl p-4 shadow-xl space-y-2">
          <div class="flex items-center justify-between text-[11px] font-mono text-slate-400 uppercase font-bold">
            <span class="flex items-center gap-1.5">
              ...
              <span>OverlayFS VFS Audit Stream:</span>
            </span>
          ...
          <div class="p-3 rounded-xl bg-[#090D14] border border-[#1D2430] font-mono text-[11px] text-slate-300 space-y-1 max-h-24 overflow-y-auto">
            <div v-for="(log, idx) in terminalLogs" :key="idx" class="truncate">
